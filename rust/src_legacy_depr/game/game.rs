...
use godot::{
    obj::Gd,
    builtin::{meta::ToGodot, Color, Variant, VariantArray, Vector2, Dictionary},
    engine::{
        Input, Node2D, NodeExt, CanvasModulate, ResourceLoader, PackedScene, CanvasLayer, AnimationPlayer, CharacterBody2D, CpuParticles2D, Node
    },
    log::godot_print
};
...
pub struct Game {
    ...
    #[serde(skip)]
    game_node: Option<Gd<Node>>,
    #[serde(skip)]
    world_map_node: Option<Gd<Node>>,
    #[serde(skip)]
    current_scene: Option<Gd<Node>>,
    #[serde(skip)]
    database: Option<Gd<Node>>,
...
impl Game {
    ...
    fn _ready(&mut self,base: &Node2D) {
        ...
        self.game_node = base.get_node(".".into());
        self.world_map_node = base.get_node("Map".into());
        ...
        self.world_map_node.unwrap().cast::<Node2D>().set_visible(false);
        base.get_node_as::<Node2D>("Player").set_visible(false);
        ...
        // self.get_external_game_data(base); // ! TODO THIS API is completly broken at this time
    }

    
    fn _process(&mut self, base: &Node2D, _delta: f64) {
        ...
        if Input::is_action_just_pressed(&self.input.unwrap(), "Menu".into()) {
        ...
        if !self.full_data_retrieved {
            ...
            if self.game_external_data.all_external_data_arrived() {
                ...
                self.world_map_node.unwrap().cast::<Node2D>().set_visible(true);
                unsafe { base.get_node_as::<Node2D>("Player").set_visible(true) };
    ...
    fn control_day_phases(&mut self, base: &Node2D) {
        if self.world_map_node.unwrap().is_inside_tree() {
            // Get's a reference to the CanvasModulate Day-Night simulator
            let mut day_night_node = base.get_node_as::<CanvasModulate>("./Map/DayNight");
            ...
            match self.game_external_data.current_dn_cycle {
                DayNightCycle::Day => { if self.current_weather == Weather::Rain {
                        day_night_node.set_deferred("color".into() , Variant::from(Color { r: 0.6, g: 0.6, b: 0.6, a: 1.0 }))
                    } else { // Setting when current weather is Weather::Sun-
                        day_night_node.set_deferred("color".into(),Variant::from(Color { r: 1.0, g: 1.0, b: 1.0, a: 1.0 }))
                    }    
                },
                DayNightCycle::Night => day_night_node.set_deferred("color".into(), Variant::from(Color { r: 0.2, g: 0.2, b: 0.3, a: 1.0 })),
    ...
    fn _save_player_position(&mut self, player_current_position: VariantArray) {
        let player_current_position: (f64, f64) = (player_current_position.get(0).to::<f64>(), player_current_position.get(1).to::<f64>());
    ...
    fn call_save_game_data_group(&self, owner: &Node2D) {
        unsafe { owner.get_tree().unwrap().call_group(
        "save_game_data".into(), "save_game_data".into(), &[]) };
    }
    ...
    fn load_initial_scene(&mut self, owner: &Node2D, path: String) {
        if !path.ends_with("Map.tscn") {
            ...
            let new_scene = ResourceLoader::singleton()
                .load(path.into()).unwrap();

            // Convert the scene resource to a Node
            self.current_scene = new_scene.cast::<PackedScene>().instantiate();

            // Insert it on the SceneTree, and set the order
            owner.add_child(self.current_scene.unwrap());
    ...
    fn change_world_scene(&mut self, base: &Node2D, path: Variant) {
        // Gets a TRef to the Node that makes the transition between scenes animation
        let scene_transition_animation = base
            .get_node_as::<CanvasLayer>("SceneTransition")
            .get_node("AnimationPlayer".into())
            .unwrap()
        ...
        if self.current_scene_path.ends_with("Map.tscn") {
            ...
            // scene_transition_animation.play("FadeToBlack", -1.0, 0.5, false); //! TODO Watch out this one

            unsafe { base.call_deferred("remove_child".into(), &[self.current_scene.unwrap().to_variant()]) };
            unsafe { base.call_deferred("add_child".into(), &[self.world_map_node.unwrap().to_variant()]) };
            
            // scene_transition_animation.play("FadeToNormal", -1.0, 1.0, false); //! TODO Watch out this one
        ...
        // Changing to an inside scene...
            ...
            unsafe { base.call_deferred("remove_child".into(), &[self.world_map_node.unwrap().to_variant()]) };

            // In order to go to a new scene, we must first load it as a resource
            let new_scene = ResourceLoader::singleton()
                .load(path.stringify()).unwrap();

            // Convert the scene resource to a Node
            self.current_scene = new_scene.cast::<PackedScene>().instantiate();
            
            // Finally we insert our new Node, setting Game as it's parent
            unsafe { base.call_deferred("add_child".into(), &[self.current_scene.unwrap().to_variant()]) };
            ...
            let player_pos = base.get_node("Player".into()).unwrap().cast::<CharacterBody2D>().get_position();
            self.current_scene.unwrap().cast::<Node2D>().set_position(
    ...
    // ! TODO THIS API is completly broken at this time
    ...
    fn set_sunrise_sunset_hours(&mut self, response: &Dictionary) {
        let sunrise_hour = response.get("sunriseHour").unwrap().to::<i32>();
        ...
        let sunset_hour = response.get("sunsetHour").unwrap().to::<i32>();
    ...
    fn set_cities_weather(&mut self, response: &Dictionary) {
        ...
        for location in self.game_cities.iter_mut() {
            let current_idx_data = current_weather.get(idx.try_into().unwrap())
                .to::<Dictionary>();
            if location.get_name() == current_idx_data.get("name")
                ...
                let location_weather_instance = CityWeather::new(
                    external_weather_data.get("weatherIDCode").unwrap().to::<i32>(),
                    external_weather_data.get("mainCode").unwrap().to::<String>(),
                    external_weather_data.get("description").unwrap().to::<String>(),
                    external_weather_data.get("icon").unwrap().to::<String>()
                );
    ...
    fn weather_control(&self, base: &Node2D) {
        if self.current_scene_type == CurrentSceneType::Outdoors {
            for location in self.game_cities.iter() {
                ...
                if let Some(mut weather_node) = base.try_get_node_as::<CpuParticles2D>(node_path) {
