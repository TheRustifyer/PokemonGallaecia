...
use crate::{game::player::PlayerCharacter, game_client::gamer::Gamer};
...
pub fn retrieve_game_data() {
    let (file, json) = open_json_file(GodotString::from_str("gamestate"), File::READ);
    let json_parse_result = json.parse(file.get_as_text()).unwrap();
    let my_data = unsafe { &json_parse_result.assume_safe().result().to_string() }; 

    let j: Result<PlayerData, serde_json::Error> = serde_json::from_str(&my_data[..]);
    
    godot_print!("RETRIEVED DATA: {:?}", &j)
}
